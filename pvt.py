...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1: #如果参数self.sr_ratio=1,attention就和vit的attention一样
...
#block的Patch emb操作，这个操作首先把224*224的图像分成4*4的一个个小patch
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            ...
            pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dims[i]))#进行位置编码
            pos_drop = nn.Dropout(p=drop_rate)#dropout正则化

...
from pvt_v2 import pvt_v2_b2,pvt_v2_b5,pvt_v2_b3
...
class ft_net_LPN(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg', block=6):
        ...
        #backbone=pvt_v2_b3
        # model_ft = pvt_v2_b3(pretrained=True)
        # model_ft.load_state_dict(torch.load("pvt_v2_b3.pth"), strict=False)

        #backbone=pvt_v2_b5
        # model_ft = pvt_v2_b5(pretrained=True)
        # model_ft.load_state_dict(torch.load("pvt_v2_b5.pth"), strict=False)

        # model for Lpn-resnet50
        # model_ft = models.resnet50(pretrained=True)
    ...
    def __init__(self, class_num, droprate, stride = 2, pool = 'avg', share_weight = False, VGG16=False, LPN=False, block=6):
        ...
        if LPN:
            ...
            if pool == 'avg+max':
                ...
                for i in range(5):
                    name = 'classifier'+str(i)
                    #setattr(self, name, ClassBlock(2048, class_num, droprate))#resnet50
                    setattr(self, name, ClassBlock(512, class_num, droprate)) #PVt-v2
        else:    
            self.classifier = ClassBlock(2048, class_num, droprate)
            if pool =='avg+max':
                self.classifier = ClassBlock(4096, class_num, droprate)

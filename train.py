...
from cal_loss import cal_triplet_loss,cal_kl_loss
...
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                             shuffle=True, num_workers=0, pin_memory=False) # 8 workers may work faster
    ...
    for epoch in range(num_epochs-start_epoch):
        ...
        for phase in ['train']:
            ...
            for data,data2,data3,data4 in zip(dataloaders['satellite'], dataloaders['street'], dataloaders['drone'], dataloaders['google']) :
                ...
                outputs_global = outputs[4]
                outputs2_global = outputs2[4]
                outputs3_global = outputs3[4]

                outputs = outputs[0:4]
                outputs2 = outputs2[0:4]
                outputs3 = outputs3[0:4]
                ...
                if not opt.LPN:
                    ...
                    if opt.views == 2:       # no implement this LPN model
                        ...
                        cls_loss = criterion(outputs_global, labels) + criterion(outputs3_global, labels3)
                        # kl_loss = cal_kl_loss(outputs_global, outputs3_global, loss_kl)
                        # kl_loss = F.kl_div(outputs_global, outputs3_global, reduction='batchmean')
                        loss = loss + 0*loss2 + loss3 + cls_loss
                        # loss = loss + loss3 + kl_loss

                        # f_triplet_loss = cal_triplet_loss(outputs_global, outputs3_global, labels,
                        #                                   triplet_loss, split_num=1)
                        # kl_loss = cal_kl_loss(outputs_global, outputs3_global, loss_kl)
                        # loss = loss + f_triplet_loss + kl_loss
                        # loss = loss + kl_loss
            ...
            if opt.views == 2:
                ...
                print('{} Loss: {:.4f} Satellite_Acc: {:.4f}  Street_Acc: {:.4f} '
                      'Drone_Acc: {:.4f}'.format(phase, epoch_loss , epoch_acc, epoch_acc2, epoch_acc3))
        ...
        file = open(r'./model/' + opt.name + '/loss.txt', "a+", encoding='utf-8')
        file.write('Epoch {}/{}'.format(epoch, num_epochs - 1) + '\n')
        file.write(
            '{} Loss: {:.4f} Satellite_Acc: {:.4f}  Street_Acc: {:.4f} Drone_Acc: {:.4f}'.format(phase, epoch_loss,
                                                                                                 epoch_acc, epoch_acc2,
                                                                                                 epoch_acc3) + '\n')

...
parser.add_argument('--name', default='Z-dataset-two_branch_res_drn-backbone+res_input-256_batchsize8_140_train-2',
...
if opt.multi:
    ...
    if scale_test:
        ...
        for x in ['query_satellite', 'query_drone', 'query_street']:
            if opt.pad > 0:
                ...
                print('----------rotation test--------------')
                image_datasets[x] = customData(os.path.join(data_dir, x), data_transforms, rotate=0)
                # print("image_datasets[x]=", image_datasets[x])
    ...
    if scale_test:
        dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                                      shuffle=False, num_workers=16) for x in
                       ['gallery_satellite', 'gallery_drone', 'gallery_street', 'gallery_satellite_usa_un',
                        'query_drone']}
    else:
        # dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
...
def extract_feature(model, dataloaders, view_index=1):
    ...
    for data in dataloaders:
        ...
        if opt.LPN:
            ...
            ff = torch.FloatTensor(n, 512, opt.block + 1).zero_().cuda()
        for i in range(2):
            if (i == 1):
                img = fliplr(img)

...
parser.add_argument('--test_dir',default='/media/user/1AD07E595CCB2146/zb/datasets/University-Release/test',type=str, help='./test_data')
parser.add_argument('--name', default='LPN+Pvt_v2+drn+two_branch-3', type=str, help='save model path')
...
parser.add_argument('--query_name', default='query_name_list.txt', type=str,help='load query image')
...
def extract_feature(model,dataloaders, view_index = 1):
    ...
    for data in dataloaders:
        ...
        if opt.LPN:
            ...
            ff = torch.FloatTensor(n,512,5).zero_().cuda()
        for i in range(2):
            if(i==1):
                img = fliplr(img)
